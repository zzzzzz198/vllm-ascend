...
__simd_vf__ void ProcessVec1UpdateImpl128VF(__ubuf__ T2 *expUb, __ubuf__ T *srcUb, __ubuf__ T *inMaxUb,
                                            ...
                                            const T minValue)
{
    ...
    AscendC::MicroAPI::Max(vreg_max_new, vreg_cur_max, vreg_in_max,
                           preg_all); // 计算新、旧max的最大值
...
__aicore__ inline void ProcessVec1UpdateImpl128(const LocalTensor<T2> &dstTensor, const LocalTensor<T> &srcTensor,
                                                ...
                                                const uint32_t originN, const T scale, const T minValue)
{
    ...
    ProcessVec1UpdateImpl128VF<T, T2, s1BaseSize, s2BaseSize>(expUb, srcUb, inMaxUb, tmpExpSumUb, tmpMaxUb, tmpMaxUb2,
